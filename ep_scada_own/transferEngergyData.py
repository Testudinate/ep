...
class DataTransmission():
		...
		if data and self.map: 
			...
			while retry<3:
				try:
					...
					logger.error('cannot connect to [%s:%s]:%s' %(self.host,self.port,e))
					return -1
			logger.info("energy item [%s] is OK." %ts)
			time.sleep(1000)
		else:
			logger.warning("energy item [%s] is lost." %ts)	
			return 0
		return 1			
	...
	def init(self): 
		...
		if delta<0: # need to be less than now
			...
			timestamp_db= time_db.timetuple()#time.strptime(time_db,'%Y-%m-%d %H:%M:%S')
	...
	def start(self):  
		...
		while 1:
			...
			if self.transfer(ts) ==-1: # cannot access to db or host:port
				logger.info("cannot access to db or [%s:%s], it will try again after 60s." %(self.host,self.port))
				time.sleep(60)
				continue
			else: 
